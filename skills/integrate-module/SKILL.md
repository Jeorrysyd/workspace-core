...
cd /path/to/workspace-core && npm start
...
```
$NOTES_DIR/                              # 笔记目录（.env 配置）
$NOTES_DIR/soul.md                       # 个人画像（可选）
data/                                    # 工作台运行时数据
server/services/ai-provider.js           # AI 调用服务（统一入口）
server/services/memory.js                # 记忆服务
```
...
3. **笔记路径** — 通过 .env 的 NOTES_DIR 配置，不要硬编码
