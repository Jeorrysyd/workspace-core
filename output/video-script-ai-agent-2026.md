...
做设计的人一听就懂——这是个 cognitive load 的问题。用户的认知资源是有限的，你的界面每多一个需要他判断的东西，他就离关掉你的产品近一步。

但现在绝大部分 agent 产品，根本没有设计这层人机之间的协作接口。所有人都在拼命让 agent 更自主、更自动。但实际卡住你上量的，是用户在旁边监管的时候，认知负荷太高，累得要死。

你真正该做的，是把人盯 agent 这件事的交互成本降下来。

怎么降？Agentic UX 里有一个核心 pattern 叫 plan-before-action：agent 动手之前，先把执行计划透出来。不是做完了再告诉你，是做之前先让你扫一眼"我打算这样做，你看行不行"。就这一步，用户的控制感回来了，焦虑砍掉一半。

然后是输出层的信息架构。agent 每做完一件事，反馈必须带三样东西：置信度——我有多大把握；溯源——我是怎么得出来的；可逆性——出了错你能一键撤回。这三样少任何一个，用户就得自己去猜。猜，就是纯亏的认知成本。

这其实是一套完整的 trust calibration 框架：让用户用最小的认知代价，判断"我在什么程度上可以信任这个 agent"。能做到这一点的产品，才能真正放量。做不到的，用户用两天就关了。
...
📊 完成度自评：9.5/10 — 最终版。设计术语精准落位（cognitive load、plan-before-action、信息架构、trust calibration），非专业段保持大白话（"累得要死""猜就是纯亏的"）。两种语感交替，既突出专业身份又不劝退非设计师观众。约750字，预计2.5分钟。
