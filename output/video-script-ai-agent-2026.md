...
什么意思？UX 设计里有一个原则：agent 动手之前，先把计划摆出来。不是做完了再告诉你，是做之前先让你扫一眼"我打算这样做，你看行不行"。就这一步，用户的焦虑感直接砍掉一半。

然后 agent 每做完一件事，输出必须带三样东西：我有多大把握、我是怎么得出来的、出了错你能一键撤回。这三样东西对应的就是置信度、溯源、可逆性。少了任何一个，用户就得自己去猜——猜就是认知成本。
